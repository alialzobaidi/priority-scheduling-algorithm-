...
public class priority_preemptive {
	...
	private static int n, st = 0, ar[], pirority[], br[], wait_time[], turn_time[], curnt_t[], f[], br_copy[];
	...
	private static void read() {
		for (int i = 0; i < n; i++) {
			System.out.println("input name ID and ArrivelTime and BursTime and pirority");
			id[i] = r.next();
			ar[i] = r.nextInt();
			br[i] = r.nextInt();
	...
	private static void pirority_p() {
		...
		while (complete != n) {
			...
			for (int i = 0; i < n; i++) {
				...
				if ((ar[i] <= st) && (f[i] == 0) && (pirority[i] < min)) {
					min = pirority[i];
					index = i;
				}
			}
	...
	private static void print() {
		...
		System.out.println("Process \t ArTime \t BTime  \t WaitTime \t priority \\t TurnTime \t complete");
		for (int i = 0; i < n; i++) {
			System.out.println("   " + id[i] + "\t\t" + ar[i] + "\t\t" + br_copy[i] + "\t\t" + wait_time[i] + "\t\t"
					+ pirority[i] + "\t\t" + turn_time[i] + "\t\t" + curnt_t[i]);
