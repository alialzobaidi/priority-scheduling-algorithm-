...
public class priority_non_preemptive_with_out_arrival_time {
	//ALi alzobaidi  _ YouTube channel -> TheNewBabil

	private static float sumwait, sumturn;
	private static int n, CpuTime, ar[], br[], pr[], wait_time[], turn_time[];
	...
	private static void read() {
		for (int i = 0; i < n; i++) {
			System.out.println("input name ID and BursTime and priority");
			id[i] = r.next();
			ar[i] = 0;
			br[i] = r.nextInt();
			pr[i] = r.nextInt();
	...
	private static void print() {
		...
		for (int i = 0; i < n; i++) {
			System.out.println("   " + id[i] + "\t\t" + ar[i] + "\t\t" + br[i] + "\t\t" + pr[i] + "\t\t" + wait_time[i]
					+ "\t\t" + turn_time[i]);
